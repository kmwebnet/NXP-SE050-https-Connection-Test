...
#define OBJID_usr_key (EX_SSS_OBJID_CUST_START + 0x10000002u) //(2u)
#define OBJID_usr_cert (EX_SSS_OBJID_CUST_START + 0x10000003u) //(3u)
...
#define DFL_CA_FILE "none"
#define DFL_CA_PATH "none"
#define DFL_CRT_FILE "none"
#define DFL_KEY_FILE "none"
#define DFL_PSK ""
#define DFL_PSK_IDENTITY "Client_identity"
#define DFL_ECJPAKE_PW NULL
#define DFL_FORCE_CIPHER 0
#define DFL_RENEGOTIATION MBEDTLS_SSL_RENEGOTIATION_ENABLED
...
sss_status_t ex_sss_entry(ex_sss_boot_ctx_t *pCtx)
{
    ...
    if (useKeysFromSM) {
        ...
        /* pex_sss_demo_tls_ctx->interCaCert will have the signer CA cert handle */
        status = sss_key_object_init(&pex_sss_demo_tls_ctx->interCaCert, &pCtx->ks);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_object_init for signer cert Failed...\n");
            return kStatus_SSS_Fail;
        }

        status = sss_key_object_get_handle(
            &pex_sss_demo_tls_ctx->interCaCert, OBJID_signer_cert);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_object_get_handle for signer cert Failed...\n");
            return kStatus_SSS_Fail;
        }
        ...
        /* pex_sss_demo_tls_ctx->pub_obj will have the our root certificate */
        status =
            sss_key_object_init(&pex_sss_demo_tls_ctx->pub_obj, &pCtx->ks);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_object_init for root Cert Failed...\n");
            return kStatus_SSS_Fail;
        }

        status = sss_key_object_get_handle(
            &pex_sss_demo_tls_ctx->pub_obj, OBJID_root_cert);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_object_get_handle for root Cert Failed...\n");
            return kStatus_SSS_Fail;
        }
    ...
    */
    ...
    size_t KeyByteLen = sizeof(certbuf);
        ...
        status = sss_key_store_get_key(
            &pCtx->ks, &pex_sss_demo_tls_ctx->interCaCert, certbuf, &KeyByteLen, &KeyBitLen);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_store_get_key for signer certificate Failed...%d\n",status);
            return kStatus_SSS_Fail;
        }

        ret = mbedtls_x509_crt_parse_der(&cacert, certbuf , KeyByteLen); 

    KeyBitLen = sizeof(certbuf) * 8;
    KeyByteLen = sizeof(certbuf);

        status = sss_key_store_get_key(
            &pCtx->ks, &pex_sss_demo_tls_ctx->pub_obj, certbuf, &KeyByteLen, &KeyBitLen);
        if (status != kStatus_SSS_Success) {
            printf(" sss_key_store_get_key for root certificate Failed...%d\n",status);
            return kStatus_SSS_Fail;
        }
